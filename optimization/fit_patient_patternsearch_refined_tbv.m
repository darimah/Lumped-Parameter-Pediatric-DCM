function results = fit_patient_patternsearch_refined_tbv(clinical, options)
if nargin < 2
    options = struct();
end
...
obj1 = @(x) objective_map_co_tbv_vector(x, params0, x_names, clinical, options.solver_options);
...
params_stage1 = vector_to_struct(params0, x_names, xbest1);
...
obj2 = @(x) objective_diameter_tbv_vector(x, params_stage1, k_names, clinical, options.solver_options);
...
sim = integrate_system_tbv(clinical, params_final, options.solver_options);
