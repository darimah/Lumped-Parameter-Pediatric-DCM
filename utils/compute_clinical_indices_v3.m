function metrics = compute_clinical_indices_v3(sim)
idx = build_state_index_v3();
...
P_vs = max(0, (V_vs - UBV_sys) / params.Cvs);
P_vp = max(0, (V_vp - UBV_pul) / params.Cvp);
...
metrics.EF_lv = 100 * (metrics.V_lv_ed - metrics.V_lv_es) / metrics.V_lv_ed;
metrics.EF_rv = 100 * (metrics.V_rv_ed - metrics.V_rv_es) / metrics.V_rv_ed;
...
metrics.P_vs = P_vs; metrics.P_vp = P_vp;
