function sim = integrate_system_v3(clinical, params, solver_options)
if nargin < 3
...
if ~isfield(solver_options, 'init_v3_settings'), solver_options.init_v3_settings = struct(); end

if ~isfield(params, 'ubv_sys_fraction')
    params.ubv_sys_fraction = 0.80;
end

T = 60 / clinical.HR;
t_end = solver_options.n_cycles * T;

x0 = build_initial_state_v3(clinical, params, solver_options.init_v3_settings);

ode_options = odeset('RelTol', solver_options.RelTol, 'AbsTol', solver_options.AbsTol, 'MaxStep', solver_options.MaxStep);
[t_full, x_full] = ode15s(@(t, x) system_rhs_v3(t, x, clinical, params), [0, t_end], x0, ode_options);
...
sim.init_mode = 'V3_venous_reservoir';
end
